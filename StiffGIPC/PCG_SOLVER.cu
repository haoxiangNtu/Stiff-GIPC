...
__global__ void PCG_add_Reduction_delta0(double*                       squeue,
                                         const __GEIGEN__::Matrix3x3d* P,
                                         const double3*                b,
                                         const __GEIGEN__::Matrix3x3d* constraint,
                                         int numbers)
{
    int idof = blockIdx.x * blockDim.x;
    int idx  = threadIdx.x + idof;
    ...
    double temp = __GEIGEN__::__v_vec_dot(__GEIGEN__::__v_M_multiply(filter_b, P[idx]),
                                          filter_b);  //filter_b.x * filter_b.x * vx + filter_b.y * filter_b.y * vy + filter_b.z * filter_b.z * vz;
...
__global__ void PCG_add_Reduction_deltaN0(double*                       squeue,
                                          const __GEIGEN__::Matrix3x3d* P,
                                          const double3*                b,
                                          double3*                      r,
                                          double3*                      c,
                                          ...
                                          int numbers)
{
    ...
    __GEIGEN__::Matrix3x3d t_constraint = constraint[idx];
    double3                t_r =
        __GEIGEN__::__M_v_multiply(t_constraint, __GEIGEN__::__minus(t_b, r[idx]));
...
__global__ void PCG_add_Reduction_deltaN(double*                       squeue,
                                         double3*                      dx,
                                         const double3*                c,
                                         double3*                      r,
                                         const double3*                q,
...
__global__ void PCG_add_Reduction_tempSum(double*        squeue,
                                          const double3* c,
                                          double3*       q,
                                          const __GEIGEN__::Matrix3x3d* constraint,
                                          int numbers)
...
__global__ void __PCG_FinalStep_UpdateC(const __GEIGEN__::Matrix3x3d* constraints,
                                        const double3* s,
                                        double3*       c,
                                        double         rate,
                                        int            numbers)
...
__global__ void __PCG_constraintFilter(const __GEIGEN__::Matrix3x3d* constraints,
                                       const double3* input,
                                       double3*       output,
                                       int            numbers)
{
...
__global__ void __PCG_Solve_AX12_b(const __GEIGEN__::Matrix12x12d* Hessians,
                                   const uint4*                    D4Index,
                                   const double3*                  c,
                                   double3*                        q,
                                   int                             numbers)
...
__global__ void __PCG_Solve_AX12_b1(const __GEIGEN__::Matrix12x12d* Hessians,
                                    const uint4*                    D4Index,
                                    const double3*                  c,
                                    double3*                        q,
                                    int                             numbers)
{
    ...
    if(threadIdx.x == 0 || GRtid == 0)
        atomicAdd((&(q[*(&(D4Index[Hid].x) + MRid / 3)].x) + MRid % 3),
                  sData[threadIdx.x]);
}

__global__ void __PCG_Solve_AXALL_b2(const __GEIGEN__::Matrix12x12d* Hessians12,
                                     ...
                                     int                             offset2)
{
    ...
    if(blockIdx.x < offset4)
    {
        ...
        double rdata = Hessians12[Hid].m[MRid][MCid]
                       * (*(&(c[*(&(D4Index[Hid].x) + vId)].x) + axisId));
        ...
        mark              = __brev(mark);
        int          clznum = __clz(mark << (warpId + 1));
        unsigned int interval = std::min(clznum, 31 - warpId);
    ...
    else if(blockIdx.x >= offset4 && blockIdx.x < offset4 + offset3)
    {
        ...
        double rdata = Hessians9[Hid].m[MRid][MCid]
                       * (*(&(c[*(&(D3Index[Hid].x) + vId)].x) + axisId));
    ...
    else if(blockIdx.x >= offset4 + offset3 && blockIdx.x < offset4 + offset3 + offset2)
    {
        ...
        double rdata = Hessians6[Hid].m[MRid][MCid]
                       * (*(&(c[*(&(D2Index[Hid].x) + vId)].x) + axisId));
    ...
    {
        int idx =
            (blockIdx.x - offset4 - offset3 - offset2) * blockDim.x + threadIdx.x;
        if(idx >= numbers1)
            return;
...
__global__ void __PCG_Solve_AX12_b2(const __GEIGEN__::Matrix12x12d* Hessians,
                                    const uint4*                    D4Index,
                                    const double3*                  c,
                                    double3*                        q,
                                    int                             numbers)
{
    ...
    double rdata = Hessians[Hid].m[MRid][MCid]
                   * (*(&(c[*(&(D4Index[Hid].x) + vId)].x) + axisId));
    ...
    unsigned int mark     = __ballot_sync(0xffffffff, bBoundary);
    mark                  = __brev(mark);
    unsigned int interval = std::min(__clz(mark << (warpId + 1)), 31 - warpId);
    //mark = interval;
...
__global__ void __PCG_Solve_AX12_b3(const __GEIGEN__::Matrix12x12d* Hessians,
                                    const uint4*                    D4Index,
                                    const double3*                  c,
                                    double3*                        q,
                                    int                             numbers)
...
__global__ void __PCG_AXALL_P(const __GEIGEN__::Matrix12x12d* Hessians12,
                              const __GEIGEN__::Matrix9x9d*   Hessians9,
                              const __GEIGEN__::Matrix6x6d*   Hessians6,
                              const __GEIGEN__::Matrix3x3d*   Hessians3,
                              const uint4*                    D4Index,
...
__global__ void __PCG_AX12_P(const __GEIGEN__::Matrix12x12d* Hessians,
                             const uint4*                    D4Index,
                             __GEIGEN__::Matrix3x3d*         P,
                             int                             numbers)
{
...
__global__ void __PCG_Solve_AX9_b(const __GEIGEN__::Matrix9x9d* Hessians,
                                  const uint3*                  D3Index,
                                  const double3*                c,
                                  double3*                      q,
                                  int                           numbers)
...
__global__ void __PCG_AX9_P(const __GEIGEN__::Matrix9x9d* Hessians,
                            const uint3*                  D3Index,
                            __GEIGEN__::Matrix3x3d*       P,
                            int                           numbers)
{
...
__global__ void __PCG_Solve_AX9_b2(const __GEIGEN__::Matrix9x9d* Hessians,
                                   const uint3*                  D3Index,
                                   const double3*                c,
                                   double3*                      q,
                                   int                           numbers)
...
__global__ void __PCG_Solve_AX6_b(const __GEIGEN__::Matrix6x6d* Hessians,
                                  const uint2*                  D2Index,
                                  const double3*                c,
                                  double3*                      q,
                                  int                           numbers)
...
__global__ void __PCG_AX6_P(const __GEIGEN__::Matrix6x6d* Hessians,
                            const uint2*                  D2Index,
                            __GEIGEN__::Matrix3x3d*       P,
                            int                           numbers)
{
...
__global__ void __PCG_Solve_AX6_b2(const __GEIGEN__::Matrix6x6d* Hessians,
                                   const uint2*                  D2Index,
                                   const double3*                c,
                                   double3*                      q,
                                   int                           numbers)
...
__global__ void __PCG_Update_Dx_R(
    const double3* c, double3* dx, const double3* q, double3* r, double rate, int numbers)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx >= numbers)
        return;
...
__global__ void __PCG_Solve_AX3_b(const __GEIGEN__::Matrix3x3d* Hessians,
                                  const uint32_t*               D1Index,
                                  const double3*                c,
                                  double3*                      q,
                                  int                           numbers)
...
__global__ void __PCG_AX3_P(const __GEIGEN__::Matrix3x3d* Hessians,
                            const uint32_t*               D1Index,
                            __GEIGEN__::Matrix3x3d*       P,
                            int                           numbers)
{
...
__global__ void __PCG_Solve_AX3_b2(const __GEIGEN__::Matrix3x3d* Hessians,
                                   const uint32_t*               D1Index,
                                   const double3*                c,
                                   double3*                      q,
                                   int                           numbers)
...
double My_PCG_add_Reduction_Algorithm(int               type,
                                      ...
                                      double            alpha = 1)
{
    ...
    {
        case 0:
            PCG_add_Reduction_force<<<blockNum, threadNum, sharedMsize>>>(
                pcg_data->squeue, pcg_data->b, numbers);
            break;
        case 1:
            PCG_add_Reduction_delta0<<<blockNum, threadNum, sharedMsize>>>(
                pcg_data->squeue, pcg_data->P, pcg_data->b, mesh->Constraints, numbers);
            break;
        case 2:
        ...
        case 3:
            PCG_add_Reduction_tempSum<<<blockNum, threadNum, sharedMsize>>>(
                pcg_data->squeue, pcg_data->c, pcg_data->q, mesh->Constraints, numbers);
            break;
        case 4:
...
double My_PCG_General_v_v_Reduction_Algorithm(
    device_TetraData* mesh, PCG_Data* pcg_data, double3* A, double3* B, int vertexNum)
{
...
void Solve_PCG_AX_B2(const device_TetraData* mesh, const double3* c, double3* q, const BHessian& BH, int vertNum)
{
    ...
    __PCG_Solve_AXALL_b2<<<blockNum, threadNum>>>(BH.H12x12,
                                                  BH.H9x9,
                                                  BH.H6x6,
                                                  BH.H3x3,
                                                  BH.D4Index,
...
void construct_P2(const device_TetraData* mesh, __GEIGEN__::Matrix3x3d* P, const BHessian& BH, int vertNum)
{
    ...
    __PCG_AXALL_P<<<blockNum, threadNum>>>(BH.H12x12,
                                           BH.H9x9,
                                           BH.H6x6,
                                           BH.H3x3,
                                           BH.D4Index,
...
void PCG_FinalStep_UpdateC(const device_TetraData* mesh,
                           double3*                c,
                           const double3*          s,
                           const double&           rate,
                           int                     vertexNum)
...
int MASPCG_Process(device_TetraData* mesh,
                   PCG_Data*         pcg_data,
                   const BHessian&   BH,
                   double3*          _mvDir,
                   int               vertexNum,
                   ...
                   double            threshold)
{
    ...
    //CUDA_SAFE_CALL(cudaDeviceSynchronize());
    delta0 = My_PCG_General_v_v_Reduction_Algorithm(
        mesh, pcg_data, pcg_data->filterTempVec3, pcg_data->preconditionTempVec3, vertexNum);

    CUDA_SAFE_CALL(cudaMemcpy(pcg_data->r,
                              pcg_data->filterTempVec3,
                              vertexNum * sizeof(double3),
                              cudaMemcpyDeviceToDevice));

    PCG_constraintFilter(mesh, pcg_data->preconditionTempVec3, pcg_data->filterTempVec3, vertexNum);

    CUDA_SAFE_CALL(cudaMemcpy(pcg_data->c,
                              pcg_data->filterTempVec3,
                              vertexNum * sizeof(double3),
                              cudaMemcpyDeviceToDevice));

    deltaN = My_PCG_General_v_v_Reduction_Algorithm(
        mesh, pcg_data, pcg_data->r, pcg_data->c, vertexNum);
    //CUDA_SAFE_CALL(cudaDeviceSynchronize());
    ...
    while(cgCounts < 3000 && deltaN > errorRate * delta0)
    {
        ...
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        {
            gipc::Timer time("preconditioning MAS");
        ...
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        deltaN = My_PCG_General_v_v_Reduction_Algorithm(
            mesh, pcg_data, pcg_data->r, pcg_data->s, vertexNum);
        //CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
int PCG_Process(device_TetraData* mesh,
                PCG_Data*         pcg_data,
                const BHessian&   BH,
                double3*          _mvDir,
                int               vertexNum,
                ...
                double            threshold)
{
    //pcg_data->MP.setPreconditioner(BH, mesh->masses, 0);
    ...
    while(cgCounts < 30000 && deltaN > errorRate * delta0)
    {
        ...
        //CUDA_SAFE_CALL(cudaMemset(pcg_data->q, 0, vertexNum * sizeof(double3)));
        {
            //gipc::Timer time("spmv old");
            Solve_PCG_AX_B2(mesh, pcg_data->c, pcg_data->q, BH, vertexNum);
        }
        {
            //gipc::Timer time("dot production old");
            double tempSum = My_PCG_add_Reduction_Algorithm(3, mesh, pcg_data, vertexNum);
            double alpha = deltaN / tempSum;
            deltaO       = deltaN;
            //deltaN = 0;
...
void PCG_Data::Malloc_DEVICE_MEM(const int& vertexNum, const int& tetrahedraNum)
{
    ...
    //int maxNum = std::max(vertexNum, tetrahedraNum);
...
void BHessian::updateDNum(const int&      tri_Num,
