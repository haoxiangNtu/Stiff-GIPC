...
void set_ee_nodedup(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_nodedup, &v, sizeof(int))); }
...
void set_ee_canon(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_canon, &v, sizeof(int))); }
...
void set_ee_vloc(const int* p) { static const int* last = (const int*)-1; if(p == last) return; last = p;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_vloc, &p, sizeof(int*))); }
...
void set_ee_nomollify(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_nomollify, &v, sizeof(int))); }
...
void set_ee_trace(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_trace, &v, sizeof(int))); }
__device__ int g_ee_tgt0 = -1; __device__ int g_ee_tgt1 = -1;
void set_ee_tgt(int a, int b){ static int la = INT_MIN, lb = INT_MIN; if(a == la && b == lb) return; la = a; lb = b;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_tgt0,&a,sizeof(int))); CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_tgt1,&b,sizeof(int))); }
...
void set_ee_detgate(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_ee_detgate, &v, sizeof(int))); }
...
void set_bvh_envpart(int v) { static int last = INT_MIN; if(v == last) return; last = v;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(g_bvh_envpart, &v, sizeof(int))); }
...
void set_self_p2g(const int* p){
    static const int* last = (const int*)-1;
    if(p == last) return;
    last = p;
