...
extern __global__ void _ccd_final_alpha_combine(double* slots, int have_ccd_pairs, double d_hat, double ccd_size, int* invalid, const int* refined_invalid, const uint32_t* d_ccd_count, frame_fsm::FrameDeviceState* frame, int ccd_capacity = 0, double cfl_factor = 0.5);
