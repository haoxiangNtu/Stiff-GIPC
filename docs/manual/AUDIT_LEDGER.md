...
| **D-022** | "μ 饱和实测表 + 14 臂抓取矩阵(0.018/0.044/0.086/0.153、62–808 N、提升 50–64 mm、≤2 mm、~15 mm)" | **全部数字在两个代码树、稳定线 CHANGELOG、以及手册指定的实测权威源中全部检索不到**,唯一出处是手册自身。§9.6 声称"有 CHANGELOG 背书",但 CHANGELOG 只有剪切台 0.600±0.008 一条;与 v0.8.5.3 记录"beaker 摩擦实验待开"也相抵触 | 全仓 grep 仅命中 `PRINCIPLES_CONTACT.md:448-453` 自身;稳定线 `CHANGELOG.md:7-41` 无 μ 扫描 | **无出处的实测表**——被当作标定依据会全面误导 | PRINCIPLES_CONTACT §5.2(**撤表**,改写为"无存档实测");§9.6 背书说法同步修正 | `grep -rn '0.153\|62–808' 工程线 稳定线 --include=*.md` — 除本手册外零命中 |
| **D-023** | "beaker 抓握场景修复后摩擦读数 **5.96 N**(法向 20 N)" | 该组数字在被引的稳定线 CHANGELOG(v0.8.5.3 节)与两仓任何文档/脚本中都不存在;CHANGELOG 记载的发布验证只有剪切台 0.600±0.008 与 266–270 滞后配对 | 稳定线 `CHANGELOG.md:15-27` | 伪造的"发布验证点" | PRINCIPLES_CONTACT(整句删除) | `grep -rn '5\.96' 稳定线/CHANGELOG.md` → 零命中 |
...
| **D-049** | "tag `v0.8.5.4` … **CHANGELOG 无条目**" | tag 所指树的 `CHANGELOG.md` **有完整 `## [0.8.5.4] — 2026-08-12` 条目**(且正是 §1.5 所引 0.00 mm / +9% 数字的出处);只有被回退的磁盘工作区里没有 | `git -C 稳定线 show v0.8.5.4:CHANGELOG.md` 行 7 / 38 / 40 | 作为"发布状态待核实"的证据链,会误导读者以为 0.8.5.4 从未写过 changelog | **未见修订记录 → §4.2 / U-07(仍存活于 `KNOWN_ISSUES.md:424`、`CHANGELOG_TIMELINE.md:531`)** | `git -C 稳定线 show v0.8.5.4:CHANGELOG.md \| sed -n '1,12p'` |
...
| **U-07** | "v0.8.5.4 CHANGELOG 无条目"——tag 树实有完整条目(D-049) | **仍存活**:`KNOWN_ISSUES.md:424`、`CHANGELOG_TIMELINE.md:531` | 中——两处均作为"发布状态待核实"的证据链在用,证据本身是错的 |
