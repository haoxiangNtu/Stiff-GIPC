...
## 5.2 μ 饱和曲线:会话实测(2026-08-11),原始日志未归档

对滑动接触验证 `Ft/Fn` 是否贴设定 μ(库仑锥饱和)的 beaker 抓取 μ 扫描,**实测于 2026-08-11(A800,稳定线 v0.8.5.3,legacy 摩擦路径)**,摘要见 [`../BEAKER_FRICTION_CAMPAIGN_2026-08-11.md`](../BEAKER_FRICTION_CAMPAIGN_2026-08-11.md)。**出处限定**:该文是事后从会话记录重建的,原始 A800 日志与运行配置未归档,手册对抗校验曾因此在两树检索不到而判为"无出处"(AUDIT_LEDGER D-022)——数字真实但**尚不能从仓库内复核**,归档/重跑登记为 OPEN_POINTS OP-038。实测(设定 μ → 滑动段 Ft/Fn):0.02 → 0.018、0.05 → 0.044、0.10 → 0.086、0.20 → 0.153;14 臂抓取矩阵(pos/stitch/force × μ 1.0/0.4 + μ 扫到 0.02 + 轻夹持)14/14 成功,提升 50–64 mm,抓内下滑 0,夹持力重夹 344–808 N / 轻夹 62–70 N。
...
6. **0.792 暂态峰值**:出自剪切台实验记录(2026-08-11),原始脚本/日志未在主仓存档(§5.3)。0.600±0.008 有稳定线 CHANGELOG 背书;**μ 饱和扫描与 14 臂抓取矩阵实测于 2026-08-11(A800),原始日志未归档,摘要已重建为 `docs/BEAKER_FRICTION_CAMPAIGN_2026-08-11.md`(OP-038 跟踪归档)**。另:剪切台实测 0.600 与 §4.6 几何平均组合律(gel/bar 自接触对应得 √0.6≈0.775)之间的界面构成疑点未闭环——实测吻合 0.6 意味着被测滑动界面有效 μ 就是 0.6(两侧同 μ,或地面路径取顶点自身 μ_gd),实验几何未存档(§5.1)。
