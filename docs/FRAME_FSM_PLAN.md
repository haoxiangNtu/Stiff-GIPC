...
- test_frame_transaction hash 多态（f2293e9f/a2d17fd0/c728be39…）定性完成：
  场景 Config 未设 multienv_mode → **merged 模式，本无位级承诺**；恢复
  muda wait 后仍漂移（排除 de-wait），关 STIFF_ABD_TIER 仍漂移（排除
  abd-tier）。run-to-run 漂移 = merged 规格内非确定（与 4090 A/B 期确认的
  merged 样本差同源）。测试真实验收 = 每进程内四比字节一致 + NaN 回滚 +
