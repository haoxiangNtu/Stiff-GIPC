#!/usr/bin/env python3
"""case_36 — full ridgeback (with finger links) + 4 hybrid grippers, one
per finger backbone.  Each finger gets a complete case_35-style hybrid
gripper (rigid sub-mesh ABD + FEM unified mesh) attached via fixed_joint;
the finger's URDF-native prismatic finger_joint controls open/close.

Architecture (per finger, ×4):

    URDF chain → finger ABD (α, blue)
                   ↓ [fixed_joint, kappa=1e3]
                hybrid rigid sub-mesh (ABD β, green)
                   ↓ [stitch_spring × 150]
                hybrid FEM unified mesh (FEM γ, red)

The 4 prismatic finger_joints (left/right arm × left/right finger) are
exposed as UI sliders.  Revolute arm joints are also exposed as before.

Compared to case_35 (single hybrid on hand link):
- 4 hybrid instances instead of 1
- attached to URDF finger ABDs (which already have prismatic open/close
  joints), so no manual fixed_joint logic for hand-tracking is needed
- collision exclusions multiplied accordingly
"""
...
def main():
    # Use case_27 soft_motion_rate (1e4) — with 4 hybrid grippers worth of
    # FEM bodies and 4×150=600 stitch springs the default 1.0 is too soft.
    # case_27 explicitly calibrated 1e4 against the case_11 native demo.
    cfg = Config(
        dt=0.020,
    ...
    prismatic_strength = float(os.environ.get("CASE36_PRISMATIC_K", "10.0"))
    for i, ji in enumerate(robot.prismatic_joints):
        eng.native.set_prismatic_strength(i, prismatic_strength)
    print(f"[case36] {len(robot.revolute_joints)} revolute joints, "
          f"{len(robot.prismatic_joints)} prismatic joints "
          f"(prismatic strength × {prismatic_strength})", flush=True)
    print(f"[case36] finalized\n", flush=True)
    ...
    def callback():
        ...
        psim.Begin("case_36 — 4 hybrid grippers")
