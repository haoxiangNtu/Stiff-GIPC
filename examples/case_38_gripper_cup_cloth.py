#!/usr/bin/env python3
"""case_38 — case_36 + cup (ABD) + cloth (FEM shell) + table (Fixed ABD).

Test scene for the soft hybrid gripper: drive the prismatic fingers to
grasp the cup, then move the arm; or sweep the gripper through the
cloth to see how well softpad+rigid contacts handle deformables.

Architecture: same as case_36 (4 hybrid grippers attached to URDF
finger ABDs) plus:
  - cup_body: ABD softgriper_cup.msh on top of table, between left/right
              hand grippers' work area
  - table:    Fixed ABD cube under cup
  - cloth:    25×25 FEM shell hanging vertically near cup or below

GUI: 1 slider per arm for prismatic open/close (drives both fingers
together), revolute joint sliders, plus standard run/step/reset.
"""
