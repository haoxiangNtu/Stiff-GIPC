#!/usr/bin/env python3
"""case_35_urdf_with_hybrid.py — full ridgeback dual-panda (OBB collision)
+ case_34 hybrid gripper welded to left_arm_hand via fixed joint.

URDF: ridgeback_softgripper_no_finger.urdf  (53 link, 37 ABD bodies after
loader skips visual-only links).  OBB-simplified collision meshes (~50
verts/link) make it cheap.  Original ridgeback geometry shows in viewport.

Architecture:
    URDF chain (chassis → ... → left_arm_hand) — 37 ABD bodies via OBB
        ↓ [fixed_joint]
    hybrid gripper rigid sub-mesh (Free ABD)
        ↓ [M3.5 chain-rule, 150 pins]
    hybrid gripper unified mesh (FEM body, softpad)

GUI: 7 sliders for left_arm_joint1..7.
"""
...
def main():
    ...
    # --- 5. FEM↔ABD coupling (default: stitch_spring like case_27) ---
    # CASE35_USE_HARD_PIN=1 → M3.5 chain-rule hard pin (kinematic projection,
    #     ABD feels FEM reactive force back through chain-rule routing).
    # CASE35_USE_HARD_PIN=0 (default, matches case_27) → stitch_spring (soft
    #     PD/motor energy: FEM pulled toward ABD-derived target each step).
    #     Spring K is governed by Config.soft_motion_rate (= 1.0 here).
    #     Both directions are coupled (FEM still pulls ABD), but spring is
    #     softer than chain-rule's enforced equality, so the joint chain has
    #     more room to move under FEM load.
    remap = np.load(RIGID_REMAP, allow_pickle=True)
    rigid_v_idx = remap['rigid_v_idx']
    n_rigid = len(rigid_v_idx)
    use_hard_pin = int(os.environ.get("CASE35_USE_HARD_PIN", "0"))
    for i in range(n_rigid):
        fem_v_global = gripper_fem_v_offset + int(rigid_v_idx[i])
        abd_anchor_v = gripper_v_offset + i
        if use_hard_pin:
            eng.native.add_fem_pin_to_abd(
                fem_v_global, abd_anchor_v, gripper_id, (0,0,0))
        else:
            eng.add_stitch_spring(
                fem_v_global, abd_anchor_v, gripper_id,
                rest_offset_world=(0.0, 0.0, 0.0))
    print(f"[case35] {'M3.5 hard-pin' if use_hard_pin else 'stitch_spring'}: "
          f"{n_rigid} couplings", flush=True)
