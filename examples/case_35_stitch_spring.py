#!/usr/bin/env python3
"""case_35_stitch_spring.py — case_35 with case_27-style STITCH SPRING coupling.

Sibling of case_35_hard_pin.py.  Both share the same URDF + hybrid gripper
setup; only the FEM↔ABD coupling differs:

  STITCH SPRING (this file): each FEM↔ABD pair is connected by a soft PD
    spring (motor energy).  FEM verts are pulled toward ABD-derived target
    positions each step but free to lag.  Spring stiffness is governed by
    Config.soft_motion_rate.  Same approach case_27_softgripper_cup uses by
    default.  The joint chain has more room to move under FEM load — at
    young 1e6 the joint moves more freely than with hard_pin (validated:
    +58% hand motion at joint6=100°).  Trade-off: at very high young (~1e8)
    the FEM oscillates against the soft spring → divergence.

Use case: when joint must drive freely against a moderate-stiffness softpad
without being locked.  Sibling hard_pin file is stricter.
...
def main():
    ...
    # --- 5. FEM↔ABD coupling: stitch_spring (case_27 style) ---
    # Each FEM↔ABD pair is a soft PD spring (motor energy).  FEM vert is
    # pulled toward ABD-derived target each step.  Spring K = soft_motion_rate.
    # Bidirectional but softer than hard pin → joint sees less FEM lock-up.
    ...
    print(f"[case35-stitch] stitch_spring: {n_rigid} couplings", flush=True)
