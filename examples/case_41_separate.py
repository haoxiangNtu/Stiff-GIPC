#!/usr/bin/env python3
"""case_41_separate — case_27 style at scale 1.0: FINE softpad
(softgriper_part2_blobal.msh) loaded AS-IS as 4 separate FEM bodies, each
...
Companion to case_41_unified.py (Strategy D PLC merge of finger.stl +
...
# case_41 separate: softpad mesh AS-IS (case_27 default, "fine")
SOFTPAD_MSH  = "/home/ps/Downloads/Stiff-GIPC-hybrid-mesh/Assets/sim_data/tetmesh/softgriper_part2_blobal.msh"
...
def main():
    ...
    cfg = Config(
        ...
        # case_41 uses original softpad mesh (sub-mm edges) → need
    ...
    # case_41 separate: simple finger_full transform (engine_link_T ×
    ...
    for finger_label in FINGER_LABELS:
        ...
        print(f"[case41] {finger_label}: softpad load t = {soft_T[finger_label][:3, 3]}",
              flush=True)

    # --- 3. case_41 separate: load softpad as 4 FEM bodies (no hybrid_rigid) ---
    fem_young = float(os.environ.get("CASE41_FEM_YOUNG", "1e6"))
    ...
    sub_n     = int(os.environ.get("CASE41_STITCH_SUB", "8"))
    nn_thresh = float(os.environ.get("CASE41_NN_THRESH_MM", "30.0")) / 1000.0
    ...
    for g in grippers:
        ...
        g['n_stitch'] = n_pairs
        total_pairs += n_pairs
        print(f"[case41] {g['label']}: finger={g['finger_id']}, "
              f"fem={g['fem_rec'].body_offset}, "
              f"{n_pairs}/{len(e_sub)} stitch (sub={sub_n}, thresh={nn_thresh*1000:.0f}mm)",
              flush=True)
    print(f"[case41] total stitch pairs: {total_pairs}", flush=True)
    ...
    # case_41 separate: NO hybrid_rigid ABD; just exclude FEM softpad
    ...
    # case_41 separate: no hybrid_rigid → no green ABD set
    ...
    for r in recs:
        ...
        # case_41 separate: pure FEM softpad (no rigid sub-mesh) → no overlay
    ...
    def callback():
        ...
        psim.Begin("case_41 separate — fine softpad FEM + finger ABD (case_27 style)")
