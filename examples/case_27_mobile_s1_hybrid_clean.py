#!/usr/bin/env python3
"""Ridgeback Dual Panda + Shirt free-fall (semi-implicit) — Case 27-S1-HYBRID-CLEAN (mobile, gripper lite, others real high-poly).
...
def main():
    ...
    # with cloth.
    #
    # Physical consequence: if a non-finger body physically reaches the cloth
    # (e.g. you drag joint sliders that swing link7/8 into it), there is NO
    # IPC barrier to push them apart -- the body passes THROUGH the cloth
    # ("ghost" body). The cloth is unaffected because nothing recorded the
    # contact. This is the user's explicit choice via add_collision_exclusion.
    #
    # The sanity-check (_edgeTriIntersectionQuery) honors the same exclusion
    # matrix, so no INTERSECT spam and line search isn't trapped backtracking.
    #
    # BVHSKIP_NO_EXCL=1 disables this opt-in (back to baseline ~370ms with
    # full arm-cloth contact, no penetration).
    if os.environ.get("BVHSKIP_NO_EXCL") != "1":
        ...
        print(f"[case27] arm-cloth exclusions added for {excl_n} non-gripper "
              ...
              f"will pass THROUGH cloth without contact -- finger bodies still "
              f"interact normally.")
    ...
    def callback():
        ...
        if _begin_window("Ridgeback Panda + Shirt Free-Fall (Case 27-S1-HYBRID-CLEAN (mobile, gripper lite, others real high-poly))"):
            if running[0]:
