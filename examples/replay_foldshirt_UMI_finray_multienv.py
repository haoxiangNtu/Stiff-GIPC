#!/usr/bin/env python3
"""Multi-environment FOLD-SHIRT replay with the UMI FINRAY soft gripper.

Sibling of replay_foldshirt_multienv.py, but the gripper is the v0.6.4 UMI
finray STRATEGY_F HYBRID soft gripper (rigid mount root + FEM fin-ray truss,
gap-0 stitch) from replay_case39_UMI_sf.py — driven by IMPEDANCE / position
control: PURE POSITION CONTROL. The continuous grip command [-1,+1] is mapped
linearly to the finger opening and each prismatic joint is driven straight to
that position (POS_K = position-drive stiffness). Grasp compliance comes from the
FEM fin-ray truss, not from a soft joint spring / external force (no force-ctrl).

  * Arm  : OBB coarse collision (ridgeback_dual_panda2_OBB.urdf) by default —
           cheaper/lighter for multi-env. GRIP_URDF=detailed for the fine arm.
  * Grip : 4 finray fingers, each = rigid ABD root (young 1e8) + FEM truss
           (young 1e7), gap-0 stitched, fixed-jointed to the finger seat.
           Per-side (L/R) assets from sim_data/umi_hybrid_sf_v800.
  * Cloth: the episode's recorded shirt (FEM 2D), one per env.
  * N envs tiled on a grid in ONE merged world (StiffGIPC has no batched solver);
    P1 set_body_groups isolates envs; per-env collision exclusions mirror UMI_sf.

Per-env line-search (S1-S4) is available via STIFF_PERENV_ALPHA=1 (+ _MASK).

Usage:
    PYTHONPATH=. CASE39ME_HEADLESS=1 CASE39ME_NUM_ENVS=4 \
        python examples/replay_foldshirt_UMI_finray_multienv.py [episode.hdf5]
    # GUI (default), per-env on:
    PYTHONPATH=. CASE39ME_NUM_ENVS=4 STIFF_PERENV_ALPHA=1 STIFF_PERENV_MASK=1 \
        python examples/replay_foldshirt_UMI_finray_multienv.py
"""
...
DEFAULT_EP = _ASSETS_DIR + "trajectories/episode_fold_shirt_umi.hdf5"
ARM_SCALE = 1.0
...
def link_visual_origin(urdf_path, link_name):
    """4x4 visual/collision <origin> of link_name (mesh-local -> link frame)."""
...
def load_finray_sides():
    """Load the per-side (L/R) unified FEM mesh + rigid_v_idx remap once."""
    sides = {}
    for side in ('L', 'R'):
...
def build_env_abd(eng, env_tf, sides, arm_tf0, ge):
    """Phase A: OBB URDF + 4 finray rigid-root ABDs (young 1e8). NO cup."""
...
def build_env_fem(eng, env, env_tf, sides, cloth_obj, cloth_T0, ge):
...
def apply_frame(robot, ej, raw, close_r=0.0):
    ...
    # PURE POSITION CONTROL. The grip command is CONTINUOUS in [-1,+1]
    # (+1 = fully open, -1 = fully closed), NOT binary — map it linearly to the
    # finger opening and drive each prismatic joint straight to that position.
    # UMI finray: the two fingers per arm have MIRRORED prismatic limits
    # (joint1 [0,+0.041], joint2 [-0.041,0]); open = end farthest from 0,
    # close = near-0 end. The joint tracks position firmly; the grasp compliance
    # comes from the FEM fin-ray truss, not from a soft joint spring.
...
def main():
    ...
    print(f"[fs-umi] episode={os.path.basename(ep)} frames={len(actions)} envs={num_envs} "
          f"arm={os.path.basename(URDF_PATH)} pos_k={pos_k}", flush=True)
    ...
    print(f"\n[fs-umi] === building {num_envs} envs ===", flush=True)
    offs = make_env_offsets(num_envs, spacing)
    t0 = time.perf_counter()
    envs = [build_env_abd(eng, o, sides, arm_tf0, ge) for o in offs]
    for env, o in zip(envs, offs):
        build_env_fem(eng, env, o, sides, cloth_obj, cloth_T0, ge)
    ...
    if num_envs > 1 and int(os.environ.get("CASE39ME_ISOLATE", "1")):
        ...
        print(f"[fs-umi] env isolation ON: {n_abd_total} ABD + {n_fem_total} FEM "
              f"-> {num_envs} groups", flush=True)
    eng.finalize()
    print(f"[fs-umi] finalized {num_envs} envs in {time.perf_counter()-t0:.1f}s "
    ...
    print(f"[fs-umi] {len(robot.revolute_joints)} rev + {len(robot.prismatic_joints)} pri\n", flush=True)
    ...
    if int(os.environ.get("CASE39ME_HEADLESS", "0")):
        ...
        for fr in range(f0, f1):
            ...
            if fr % 20 == 0:
                ...
                print(f"[fs-umi-hl] frame {fr:4d} step={ms[-1]:6.0f}ms cloth_y/env={['%+.3f'%z for z in cz]}", flush=True)
        mm = float(np.mean(ms))
        print(f"\n[fs-umi-hl] {num_envs} envs, {len(ms)} frames: mean {mm:.1f}ms "
