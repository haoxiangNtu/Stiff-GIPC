...
if [ "$TIER" = "heavy" ]; then
    echo "== HEAVY production FEM-model build matrix =="
    bash "$ROOT/scripts/model_build_matrix.sh" || exit 1

    echo "== HEAVY FD matrix =="
    ...
    BENCH_REQUIRE_BASELINE=1 BENCH_REQUIRE_IDLE_GPU=1 \
        python3 "$ROOT/scripts/mode_bench.py" || exit 1
