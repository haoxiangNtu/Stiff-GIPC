...
SOFTPAD = REPO / "Assets/sim_data/tetmesh/softgriper_part2_blobal.msh"   # case_27 fine
OUT_UNIFIED = REPO / "Assets/sim_data/hybrid_d/CASE41_unified.npz"
OUT_RIGID   = REPO / "Assets/sim_data/hybrid_d/CASE41_rigid.msh"
OUT_REMAP   = REPO / "Assets/sim_data/hybrid_d/CASE41_rigid_remap.npz"
MAXVOL  = 1.2e-6   # FINE-ish — 25% smaller than CASE37 (1.5e-6).  Going
                   # finer (3e-7 .. 1e-6) caused CUDA crash at scale 1.0
                   # because absolute edge lengths sub-100μm trigger CCD
                   # divergence with relative_dhat=1e-3 (gapl >> edge).
...
def main():
    ...
    keep = vols >= 5e-11  # tighter cutoff for fine mesh — drops more
                          # near-degenerate tets (kept 1e-11 for CASE37/40)
    n_drop = int((~keep).sum())
    if n_drop:
        tets = tets[keep]
